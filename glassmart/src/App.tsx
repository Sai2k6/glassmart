...
function App() {
  ...
  const runSearch = () => { if (!searchQuery.trim()) return; navigate("search"); };
  ...
  const Header = () => <>
    <header className="navbar">
      ...
      {userRole !== "admin" && <div className="search-container"><select className="search-category" value={searchCategory} onChange={e => setSearchCategory(e.target.value)}><option>All</option><option>Glass</option><option>Hardware</option></select><input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} onKeyDown={e => e.key === "Enter" && runSearch()} placeholder="Search Glassmart..." className="search-input"/><button className="search-button" onClick={runSearch}>⌕</button></div>}
