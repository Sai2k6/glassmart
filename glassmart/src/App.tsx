...
function App() {
  ...
  const addHardwareToCart = (brand: HwBrandCard, groupName: string, categoryName: string) => {
    ...
    setCart(current => {
      ...
      return existing ? current.map(i => i.id === key ? { ...i, quantity: Math.min(i.quantity + 1, brand.stock), stock: brand.stock } : i) : [...current, { id: key, name: brand.brand, description: `${categoryName} · ${groupName}`, price: HARDWARE_MRP, quantity: 1, image: brand.image, category: categoryName, kind: "hardware", options, stock: brand.stock }];
    });
