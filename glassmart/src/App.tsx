...
function App() {
  ...
  const searchHardware = useMemo(() => {
    ...
    hardwareGroups.forEach(g => g.categories.forEach(c => getBrandCards(c).forEach(b => {
  ...
  return <div className="app"><Header />
    ...
    {page === "confirmation" && <main className="page-container"><div className="success"><div className="success-icon">✓</div><p className="eyebrow">ORDER PLACED</p><h1>Thank you for your order!</h1><p>Your hardware order has been saved successfully.</p>{lastOrder && <p className="order-number">Order ID: {lastOrder.id}</p>}<div className="hero-buttons"><button className="primary-btn" onClick={() => navigate("orders")}>View Orders</button><button className="secondary-btn" onClick={goHardwareHome}>Continue Shopping</button></div></div></main>}

    {page === "orders" && <main className="page-container"><div className="page-title"><p className="eyebrow">ORDER HISTORY</p><h1>Your Orders</h1></div>{ordersLoading ? <div className="loading">Loading orders…</div> : !orders.length ? <div className="empty-state"><h2>No orders yet.</h2><p>Your hardware orders will appear here.</p><button className="primary-btn" onClick={goHardwareHome}>Browse Hardware Fittings</button></div> : <div className="orders-list">{orders.map(o => <article className="order-card" key={o.id}><div className="order-card-head"><div><strong>Order {o.id.slice(0, 8)}</strong><span>{new Date(o.created_at).toLocaleString("en-IN")}</span></div><b>{o.status}</b></div><div>{o.items.map(i => <div className="order-item" key={i.id}><span>{i.name} × {i.quantity}</span><strong>{money(i.price * i.quantity)}</strong></div>)}</div><div className="summary-total"><span>Total</span><strong>{money(o.total)}</strong></div></article>)}</div>}</main>}

    {page === "services" && <main className="page-container"><div className="page-title"><p className="eyebrow">SERVICES</p><h1>Need a custom solution?</h1><p>Tell us what you need and our team can follow up.</p></div><form className="service-form" onSubmit={submitEnquiry}><div className="form-grid"><label>Name<input required value={enquiry.name} onChange={e => setEnquiry(v => ({ ...v, name: e.target.value }))}/></label><label>Phone<input required value={enquiry.phone} onChange={e => setEnquiry(v => ({ ...v, phone: e.target.value }))}/></label><label>Email<input required type="email" value={enquiry.email} onChange={e => setEnquiry(v => ({ ...v, email: e.target.value }))}/></label><label>Service<select required value={enquiry.service} onChange={e => setEnquiry(v => ({ ...v, service: e.target.value }))}><option value="">Select a service</option>{SERVICES.map(s => <option key={s}>{s}</option>)}</select></label></div><label>Requirements<textarea required value={enquiry.requirements} onChange={e => setEnquiry(v => ({ ...v, requirements: e.target.value }))} placeholder="Describe your requirements..."/></label>{enquiryMessage && <p className={enquiryMessage.startsWith("Could not") ? "form-error" : "form-success"}>{enquiryMessage}</p>}<button className="primary-btn" disabled={enquiryLoading}>{enquiryLoading ? "Submitting…" : "Submit Enquiry"}</button></form></main>}
    ...
    {page === "login" && <main className="page-container auth-page"><div className="auth-card"><p className="eyebrow">GLASSMART ACCOUNT</p><h1>{authMode === "login" ? "Welcome back" : "Create your account"}</h1><form onSubmit={handleAuth}>{authMode === "register" && <><label>Name<input required value={authName} onChange={e => setAuthName(e.target.value)}/></label><label>Phone<input required value={authPhone} onChange={e => setAuthPhone(e.target.value)}/></label><label>Account type<select value={accountType} onChange={e => setAccountType(e.target.value as "customer" | "carpenter")}><option value="customer">Customer</option><option value="carpenter">Carpenter</option></select></label></>}<label>Email<input required type="email" value={authEmail} onChange={e => setAuthEmail(e.target.value)} autoComplete="email"/></label><label>Password<input required type="password" value={authPassword} onChange={e => setAuthPassword(e.target.value)} autoComplete={authMode === "login" ? "current-password" : "new-password"}/></label>{authMode === "register" && <label>Confirm password<input required type="password" value={authConfirm} onChange={e => setAuthConfirm(e.target.value)} autoComplete="new-password"/></label>}{authError && <p className="form-error">{authError}</p>}<button className="primary-btn full" disabled={authLoading}>{authLoading ? "Please wait…" : authMode === "login" ? "Login" : "Create Account"}</button></form><button className="text-btn" onClick={() => { setAuthMode(v => v === "login" ? "register" : "login"); setAuthError(""); }}>{authMode === "login" ? "New to Glassmart? Create an account" : "Already have an account? Login"}</button></div></main>}

    {page === "account" && <main className="page-container"><div className="account-head"><div><p className="eyebrow">MY ACCOUNT</p><h1>{user?.user_metadata?.name || user?.email}</h1><p>{user?.email}</p><span className="role-pill">{userRole || "customer"}</span></div><button className="secondary-btn" onClick={logout}>Logout</button></div><div className="account-grid"><button className="account-tile" onClick={() => { loadOrders(); navigate("orders"); }}><h2>Orders</h2><p>Track your hardware orders.</p></button><button className="account-tile" onClick={goHardwareHome}><h2>Hardware Catalogue</h2><p>Continue browsing hardware fittings.</p></button><button className="account-tile" onClick={() => navigate("services")}><h2>Enquiry</h2><p>Send a custom requirement.</p></button></div></main>}

    {page === "admin" && userRole === "admin" && <main className="page-container admin-page"><div className="page-title"><p className="eyebrow">ADMIN</p><h1>Glassmart Admin</h1><p>Manage catalogue data, orders and enquiries.</p></div><div className="admin-grid"><button className="account-tile" onClick={() => navigate("admin-products")}><h2>Products</h2><p>Manage the hidden glass product database.</p></button><button className="account-tile" onClick={() => navigate("admin-orders")}><h2>Orders</h2><p>Review customer orders.</p></button><button className="account-tile" onClick={() => navigate("admin-enquiries")}><h2>Enquiries</h2><p>Review service enquiries.</p></button><button className="account-tile" onClick={() => navigate("admin-hardware")}><h2>Hardware</h2><p>Review the hardware catalogue.</p></button></div></main>}

    {page === "admin-products" && userRole === "admin" && <main className="page-container admin-page"><div className="page-title"><button className="back-link" onClick={() => navigate("admin")}>← Admin</button><p className="eyebrow">PRODUCT DATABASE</p><h1>Glass Products (hidden from customers)</h1></div><div className="admin-toolbar"><button className="primary-btn" onClick={() => { setEditingProduct(null); setAdminProduct({ name: "", description: "", mrp: "", offer_price: "", stock: "", category: "", image_url: "" }); setShowProductForm(true); }}>+ Add Product</button></div>{showProductForm && <form className="admin-form" onSubmit={saveAdminProduct}><label>Name<input required value={adminProduct.name} onChange={e => setAdminProduct(v => ({ ...v, name: e.target.value }))}/></label><label>Description<textarea value={adminProduct.description} onChange={e => setAdminProduct(v => ({ ...v, description: e.target.value }))}/></label><div className="form-grid"><label>MRP<input type="number" min="0" value={adminProduct.mrp} onChange={e => setAdminProduct(v => ({ ...v, mrp: e.target.value }))}/></label><label>Offer price<input type="number" min="0" value={adminProduct.offer_price} onChange={e => setAdminProduct(v => ({ ...v, offer_price: e.target.value }))}/></label><label>Stock<input type="number" min="0" value={adminProduct.stock} onChange={e => setAdminProduct(v => ({ ...v, stock: e.target.value }))}/></label><label>Category<select value={adminProduct.category} onChange={e => setAdminProduct(v => ({ ...v, category: e.target.value }))}><option value="">Select category</option>{PRODUCT_CATEGORIES.map(c => <option key={c}>{c}</option>)}</select></label></div><label>Image URL<input value={adminProduct.image_url} onChange={e => setAdminProduct(v => ({ ...v, image_url: e.target.value }))}/></label><div className="modal-actions"><button type="button" className="secondary-btn" onClick={() => setShowProductForm(false)}>Cancel</button><button className="primary-btn">{editingProduct ? "Update Product" : "Save Product"}</button></div></form>}{adminMessage && <p className="form-success">{adminMessage}</p>}<div className="admin-list">{adminProducts.length ? adminProducts.map(p => <article className="admin-row" key={p.id}><div><strong>{p.name}</strong><span>{p.category} · stock {p.stock} · MRP {money(p.mrp || p.price || 0)}</span></div><div className="row-actions"><button className="secondary-btn" onClick={() => openEdit(p)}>Edit</button><button className="danger-btn" onClick={() => deleteAdminProduct(p)}>Delete</button></div></article>) : <div className="empty-state"><h2>No products loaded.</h2></div>}</div></main>}

    {page === "admin-orders" && userRole === "admin" && <main className="page-container admin-page"><div className="page-title"><button className="back-link" onClick={() => navigate("admin")}>← Admin</button><p className="eyebrow">ORDER MANAGEMENT</p><h1>Orders</h1></div><div className="empty-state"><h2>Use the Orders page for customer order history.</h2><p>The existing database-backed order flow remains active.</p></div></main>}
    {page === "admin-enquiries" && userRole === "admin" && <main className="page-container admin-page"><div className="page-title"><button className="back-link" onClick={() => navigate("admin")}>← Admin</button><p className="eyebrow">ENQUIRIES</p><h1>Service Enquiries</h1></div><div className="empty-state"><h2>Enquiries are stored in Supabase.</h2><p>Connect an enquiries listing here when you want to add admin-side review.</p></div></main>}
    {page === "admin-hardware" && userRole === "admin" && <main className="page-container admin-page"><div className="page-title"><button className="back-link" onClick={() => navigate("admin")}>← Admin</button><p className="eyebrow">HARDWARE CATALOGUE</p><h1>Hardware structure</h1><p>Hardware remains the active customer catalogue.</p></div><div className="hw-grid">{hardwareGroups.map(g => <div className="hw-card" key={g.id}><span className="hw-card-name">{g.name}</span><span className="hw-card-meta">{g.categories.map(c => c.name).join(" · ")}</span></div>)}</div></main>}

    {userRole !== "admin" && <footer className="footer"><div><button className="logo footer-logo" onClick={() => navigate("home")}><span>GLASS</span>MART</button><p>Premium hardware fittings for modern spaces.</p></div><div><strong>Catalogue</strong><button onClick={goHardwareHome}>Hardware Fittings</button><button onClick={() => navigate("services")}>Services</button><button onClick={() => navigate("contact")}>Contact</button></div><div><strong>Account</strong><button onClick={() => user ? navigate("account") : navigate("login")}>Login / Account</button><button onClick={() => { if (user) { loadOrders(); navigate("orders"); } else navigate("login"); }}>Orders</button><button onClick={() => navigate("cart")}>Cart</button></div></footer>}
